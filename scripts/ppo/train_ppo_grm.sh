...
reward_peft_path='reward_models/model_finetuned_proxy_grm_2b/logs/checkpoint-1250' 
wandb_name="ppo_grm2B_lr1e-5_klreg0.0_normrewards"
CUDA_VISIBLE_DEVICES=${gpu} accelerate launch --main_process_port 10009 --num_processes 4 ppo_grm.py \
...
CUDA_VISIBLE_DEVICES=${gpu} accelerate launch --main_process_port 10008 --num_processes 4 ppo_grm.py \
    --base_model_name ${base_model_name} \
    --reward_base_model ${reward_base_model} \
    --reward_peft_path ${reward_peft_path} \
    --dataset_path ${dataset_path}\
