...
# trl >= 0.29 moved PreTrainedModelWrapper to trl.experimental.ppo.
# HuggingFace-hosted model.py files (e.g. GRM-Gemma-2B-sftreg) still do
# ``from trl import PreTrainedModelWrapper``, so patch the name back.
import trl as _trl
if not hasattr(_trl, "PreTrainedModelWrapper"):
    from trl.experimental.ppo import PreTrainedModelWrapper as _PTMW
    _trl.PreTrainedModelWrapper = _PTMW
...
def load_reward_model(
    ...
    *,
    ...
    ensure_fla_kernels(model_name, trust_remote_code=trust_remote_code)
    if reasoning:
...
def _run_batched_pairwise_comparisons(
    ...
    return all_preferences, prompts_for_model, generated_texts
...
def get_reward_reasoning(
    ...
    for prompt, group in grouped_completions.items():
        ...
        tournaments_data.append({
            ...
            'players_state': [{'id': i, 'score': 0, 'opponents': set(), 'generations': [], 'prompts': []} for i in range(num_players)],
            'all_matches': [],
        })
    ...
    for round_num in range(num_total_rounds):
        ...
        preferences, prompts_for_rm, generations = _run_batched_pairwise_comparisons(
        ...
        for i, request in enumerate(requests_this_round):
            ...
            player1_state['prompts'].append(prompts_for_rm[i])
            player2_state['prompts'].append(prompts_for_rm[i])
    ...
    if reward_controller is not None:
        if reward_controller.trainer.state.global_step % reward_controller.trainer.state.logging_steps == 0:
            ...
            df = pd.DataFrame({
                ...
                'completions': winner_completions,
