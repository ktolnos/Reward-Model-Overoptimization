...
# for UnifiedFeedback
def build_dataset_UF(
    data_path, tokenizer, split="train", size=None, mode="", model_name=""
):
    ...
    def formatting_func(example):
        ...
        if "summarize" in example["source"]:
            chosen_messages[0]["content"] = (
                "Generate one-sentence summary for the following post: "
                + chosen_messages[0]["content"].strip()
            )
            rejected_messages[0]["content"] = (
                "Generate one-sentence summary for the following post: "
                + rejected_messages[0]["content"].strip()
            )
...
def load_train_eval_dataset(
    ...
    if "Unified" in data_path:
        ...
        eval_dataset = build_dataset_UF(
            data_path, tokenizer, split="val", model_name=model_name
        )
    else:
