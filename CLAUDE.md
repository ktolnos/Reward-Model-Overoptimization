- Use `venv/bin/python` and `venv/bin/python -m pip` instead of globally installed `python` and `pip`.
- You are most likely not on the cluster, but on the local machine. Don't try to submit slurm jobs or look for /nas paths.
